...
async function simplifyExpression() {
    ...
    try {
        ...
        const formatInstruction = [
            "输出格式必须严格如下：",
            "---",
            "（这里放置计算/化简/求解过程，每一步一行，使用LaTeX格式）",
            "---",
            "===",
            "（这里放置最终结果，使用LaTeX格式）",
            "===",
            "不要输出任何其他内容，包括解释、评论或广告。"
        ].join('\n');
        ...
        if (currentMode === 'calculator') {
            taskDescription = [
                ...
                "计算过程只需展示逐步计算，无需文字说明。",
                "计算过程示例：{原式}{换行}={第一步}{换行}=...，以此类推，每步一行（过程的第一行单独显示原式，过程的第二行显示第一步，等号不要单独占一行！）。",
            ...
            if (allowImaginary) {
                taskDescription = [
                    ...
                    "求解过程只需展示逐步变形，无需文字说明。",
                    "计算过程示例：3x+5=11{换行}3x=6{换行}...=...，以此类推，每步一行。",
                ...
                taskDescription = [
                    "你的任务：接收一个 LaTeX 方程或不等式，求解未知数。",
                    "求解过程只需展示逐步变形，无需文字说明。",
                    "计算过程示例：3x+5=11{换行}3x=6{换行}...=...[部分多次方程要用到判别式：“Δ=...”]，以此类推，每步一行。",
                    "如果是多个根，用 x_1=..., x_2=... 的形式。",
